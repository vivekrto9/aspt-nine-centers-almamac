...
test("repository docs describe Nine Centres Field Almanac without stale single-page or legacy catalog guidance", () => {
  const publicDocs = Object.values(docs).join("\n");
  const d1Schema = read("database/d1/001_initial_site_schema.sql");

  assert.match(docs.readme, /# Nine Centres — Field Almanac/);
  assert.match(docs.readme, /Field Almanac Human Design template/);
  ...
  assert.doesNotMatch(publicDocs, /templates\/aspt-nine-centers-almamac\/0\.1\.0\//);
...
test("lead documentation is an agent-ready integration reference", () => {
  ...
  assert.match(docs.leads, /wrangler d1 execute aspt-nine-centers-almamac-site --local/);
